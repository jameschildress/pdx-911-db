describe PDX911::Scraper do
  ...
  describe "run()" do
    ...
    it "creates a Dispatch for every entry" do
      ...
      @dispatches[0].category.must_equal  @categories[0]
      @dispatches[0].agency.must_equal    @agencies[0]
      @dispatches[0].location.must_equal  '(45.519650, -122.682966)'
      Time.parse(@dispatches[0].date).must_equal Time.parse('2013-12-16T16:04:09.0-08:00')
      
      @dispatches[1].uid.must_equal       'PG13000069877'
      @dispatches[1].address.must_equal   '19400 BLOCK OF E BURNSIDE ST, GRESHAM, OR'
      @dispatches[1].category.must_equal  @categories[1]
      @dispatches[1].agency.must_equal    @agencies[1]
      @dispatches[0].location.must_equal  '(45.517010, -122.463297)'
      Time.parse(@dispatches[1].date).must_equal Time.parse('2013-12-16T16:01:01.0-08:00')
      
      @dispatches[2].uid.must_equal       'PP13000410477'
      @dispatches[2].address.must_equal   '1100 BLOCK OF NE 80TH AVE, PORTLAND, OR'
      @dispatches[2].category.must_equal  @categories[1]
      @dispatches[2].agency.must_equal    @agencies[0]
      @dispatches[0].location.must_equal  '(45.531446, -122.580687)'
      Time.parse(@dispatches[2].date).must_equal Time.parse('2013-12-16T15:31:09.0-08:00')
      
      @dispatches[3].uid.must_equal       'PP13000410491'
      @dispatches[3].address.must_equal   '17100 BLOCK OF SE POWELL BLVD, PORTLAND, OR'
      @dispatches[3].category.must_equal  @categories[0]
      @dispatches[3].agency.must_equal    @agencies[0]
      @dispatches[0].location.must_equal  '(45.491989, -122.488314)'
      Time.parse(@dispatches[3].date).must_equal Time.parse('2013-12-16T15:23:32.0-08:00')
    end
